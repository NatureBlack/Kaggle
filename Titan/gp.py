...
def Outputs(d):
    return np.around((1 - (1 / (1 + np.exp(-d)))))
...
if __name__ == "__main__":
    ...
    geneticTrain = GeneticFunction(mungedtrain)
    trainPredictions = Outputs(geneticTrain.values)
    ...
    genetic = GeneticFunction(mungedtest)
    testPredictions = Outputs(genetic.values)
